...
O coletor utiliza numeração **BCM** para o GPIO. Na instalação do Colégio João XXIII, o sinal foi ligado ao GPIO17. Esse número **não está fixado diretamente no código**: ele deve ser informado pela variável de ambiente `RAIN_GPIO_PIN`.

```text
RAIN_GPIO_PIN=17
```

Se `RAIN_GPIO_PIN` não estiver configurado, o próprio coletor registra que o pino não foi definido e mantém o pluviômetro desativado.
...
O coletor atual possui `0.297` como valor de fallback de `RAIN_MM_PER_PULSE` quando a variável não é definida ou quando o valor informado não pode ser convertido corretamente.

A calibração documentada pela equipe, entretanto, resultou em aproximadamente `0.29205 mm` por basculada. Por isso, para reproduzir essa calibração, a configuração deve ser fornecida explicitamente:

```text
RAIN_MM_PER_PULSE=0.29205
```

Se a geometria da área de captação for alterada, esse valor deve ser recalculado e validado novamente.

## Fluxo do coletor atual

No código atual, a inicialização e a coleta seguem, de forma resumida, este fluxo:

1. cria ou verifica o banco SQLite local;
2. configura o pluviômetro a partir de `RAIN_GPIO_PIN`;
3. lê o `rcID` da estação;
4. tenta atualizar IP e MAC da estação no MariaDB;
5. lê BME280 e BH1750 em ciclos de aproximadamente 10 segundos;
6. contabiliza de forma assíncrona os pulsos recebidos do pluviômetro;
7. a cada 300 segundos, calcula as médias dos sensores e a precipitação acumulada;
8. grava o registro no SQLite;
9. tenta sincronizar os registros pendentes com o MariaDB.

A precipitação é calculada antes do salvamento pela expressão:

```text
rain_mm = pulsos_do_intervalo × RAIN_MM_PER_PULSE
```
...
O coletor atual lê as configurações do pluviômetro por variáveis de ambiente:
...
- `RAIN_GPIO_PIN` é necessário para ativar o pluviômetro no software. Na instalação do Colégio João XXIII foi utilizado o GPIO17.
- `RAIN_MM_PER_PULSE` define o fator de conversão. Se não for informado, o código utiliza `0.297` como fallback.

As configurações do MariaDB também são obtidas pelo ambiente:
...
Além disso, a identificação da estação é lida de:

```text
~/.config/station/rcid.txt
```

> Os documentos herdados descrevem versões do instalador que inseriam credenciais diretamente no script Python. O coletor atual lê essas configurações por variáveis de ambiente. Portanto, ao utilizar o instalador legado, deve-se confirmar que o serviço instalado recebe essas variáveis corretamente.

## Observação sobre uso isolado do pluviômetro

O subsistema foi documentado de forma reutilizável, porém o **coletor atual ainda depende da estação herdada durante a inicialização**.

No código existente, se **BME280 e BH1750 não forem detectados ao mesmo tempo**, o programa encerra a execução antes de entrar no loop principal. Portanto, para utilizar somente o pluviômetro em um Raspberry Pi sem os sensores herdados, essa verificação precisa ser adaptada.

Isso não altera a ligação elétrica ou a lógica do pluviômetro; é apenas uma restrição da versão atual do script de coleta.
...
Como ferramenta de diagnóstico, é possível consultar os últimos registros do buffer:
...
Como exemplo de diagnóstico, para filtrar eventos do pluviômetro nos logs:

```bash
journalctl -u NOME_DO_SERVICO.service | grep -i "pluviômetro"
```

## Observações técnicas do código atual

- O SQLite armazena um campo `created_at` com a data e hora local do registro. Na rotina atual de sincronização, o `INSERT` no MariaDB envia `rcID`, `Temp`, `Humidity`, `Pressure`, `Lux` e `Pluv`; o campo `created_at` do buffer não é enviado explicitamente.
- O coletor compara a leitura atual com `last_saved_data`. Se todos os valores calculados forem idênticos ao registro anterior, o novo salvamento no SQLite é ignorado como duplicado.
- O contador de pulsos do pluviômetro é lido e zerado no início de cada janela de 5 minutos, antes da tentativa de persistência. Por isso, a disponibilidade do SQLite local continua sendo importante para a robustez do processo.

Esses pontos descrevem o comportamento da versão atual e podem ser revistos em futuras melhorias do coletor.
